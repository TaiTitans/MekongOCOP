    ...
    <div class="flex flex-col min-h-screen mx-auto bg-gray-100">
        ...
        <table class="min-w-full bg-white border border-gray-200 rounded">
          ...
          <tbody>
            <tr v-for="seller in paginatedSellers" :key="seller.seller_id">
              ...
              <td class="py-2 px-4 border-b">{{ seller.status === 'Pending' ? 'Đang chờ duyệt' : seller.status === 'Accept' ? 'Đã duyệt' : seller.status }}</td>
              ...
              <td class="py-2 px-4 border-b">
                <button
                  ...
                  class="px-4 py-2 bg-white-500 text-black rounded-lg hover:bg-gray-200 shadow-md"
                >
                  Duyệt
                </button>
              </td>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-lg shadow-lg relative">
          <button @click="closeModal" class="absolute top-1 right-2 text-gray-500 text-lg font-bold">
            X
          </button>
          <img :src="selectedImage" alt="Chứng Nhận" class="w-96 h-auto rounded" />
        </div>
      </div>
