...
# Đường dẫn file H5
h5_file = 'chatbot_data_phobert_cnn.h5'
...
class CNNModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.fc1 = nn.Linear(128 * (input_size // 2), num_classes)
...
num_classes = len(label_encoder.classes_)  # Số lượng lớp phải khớp với số lượng lớp trong LabelEncoder
num_epochs = 20
batch_size = 32
learning_rate = 0.001
...
for train_index, test_index in kf.split(question_embeddings):
   ...
   # Chuyển đổi dữ liệu thành tensor và thêm chiều batch
    train_questions = torch.tensor(train_questions, dtype=torch.float32).to(device)
    test_questions = torch.tensor(test_questions, dtype=torch.float32).to(device)
    ...
    train_questions = train_questions.unsqueeze(1)  # [batch_size, 1, sequence_length]
    test_questions = test_questions.unsqueeze(1)    # [batch_size, 1, sequence_length]
